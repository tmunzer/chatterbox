    ...
    Acs
        ...
        .exec(function (err, accounts) {
            ...
            // for every selected accounts, try to refresh the token
                ...
                if (account.slack == [] && account.spark == []) {
                    console.info("\x1b[32minfo\x1b[0m:", "Account for ownerId " + account.ownerId + " is no more used. Removing it");
                    Acs.remove(account, function (err) {
                        if (err) console.error("\x1b[31mERROR\x1b[0m:", err);
                    })
                }
                else {
